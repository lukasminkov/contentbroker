...
const Home = () => {
  ...
  const totalRetainersEarned = campaigns?.filter(campaign => 
    campaign.status === 'completed'
  ).reduce((sum, campaign) => sum + Number(campaign.amount), 0) || 0

  const totalRetainersInProgress = campaigns?.filter(campaign => 
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-[#7E69AB]">
                  ${totalRetainersInProgress.toLocaleString()}
                </span>
              </div>
            </div>
          </CardContent>
